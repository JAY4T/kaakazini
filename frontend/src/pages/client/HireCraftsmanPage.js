...
@keyframes hcp-fadeUp  { from { opacity:0; transform:translateY(16px); } to { opacity:1; transform:translateY(0); } }
...
.hcp .sb {
  ...
  flex-shrink: 0; border-right: 1px solid rgba(255,215,0,.08);
...
.hcp .user-badge { display: inline-flex; align-items: center; gap: 4px; background: rgba(255,215,0,.12); color: #FFD700; border: 1px solid rgba(255,215,0,.25); border-radius: 20px; padding: 2px 8px; font-size: .58rem; font-weight: 700; text-transform: uppercase; letter-spacing: .08em; }
.hcp .user-badge::before { content: ''; width: 5px; height: 5px; border-radius: 50%; background: #FFD700; }
.hcp .sb-nav { position: relative !important; z-index: 1 !important; flex: 1 !important; padding: 14px 10px !important; overflow-y: auto !important; display: flex !important; flex-direction: column !important; }
...
.hcp .nb { width: 100% !important; padding: 10px 12px !important; border: none !important; border-radius: 10px !important; background: transparent !important; color: rgba(255,255,255,.5) !important; font-family: 'Outfit', sans-serif !important; font-weight: 600 !important; font-size: .83rem !important; text-align: left !important; display: flex !important; align-items: center !important; gap: 10px !important; cursor: pointer !important; transition: all .18s !important; margin-bottom: 2px !important; position: relative !important; }
.hcp .nb-icon { width: 28px !important; height: 28px !important; border-radius: 7px !important; background: rgba(255,255,255,.07) !important; display: flex !important; align-items: center !important; justify-content: center !important; flex-shrink: 0 !important; transition: all .18s !important; font-size: .78rem !important; }
.hcp .nb:hover { color: #fff !important; }
.hcp .nb:hover .nb-icon { background: rgba(255,215,0,.15); color: #FFD700; }
.hcp .nb.on { color: #fbbf24 !important; }
...
.hcp .srch:focus { border-color: #FFD700; box-shadow: 0 0 0 4px rgba(255,215,0,.1); }
...
.hcp .trade-pill { position: absolute; top: 10px; left: 10px; z-index: 1; background: linear-gradient(135deg, #FFD700, #e6c200); color: #0d0d0d; border-radius: 50px; padding: 3px 11px; font-size: .6rem; font-weight: 800; letter-spacing: .04em; box-shadow: 0 2px 8px rgba(255,215,0,.4); }
.hcp .avail-pill { position: absolute; top: 10px; right: 10px; z-index: 1; background: #16a34a; color: #fff; border-radius: 50px; padding: 3px 9px; font-size: .58rem; font-weight: 700; display: flex; align-items: center; gap: 4px; box-shadow: 0 2px 8px rgba(22,163,74,.4); }
.hcp .avail-dot { width: 5px; height: 5px; border-radius: 50%; background: #fff; animation: hcp-pulse 1.5s infinite; }
...
.hcp .craft-av { width: 40px; height: 40px; border-radius: 50%; object-fit: cover; border: 2.5px solid #FFD700; flex-shrink: 0; }
.hcp .craft-name { font-weight: 800; font-size: .9rem; color: var(--text); }
.hcp .craft-loc { font-size: .72rem; color: #16a34a; font-weight: 600; margin-top: 2px; }
...
.hcp .back-btn:hover { border-color: #FFD700; color: var(--text); background: #fef9c3; }
...
.hcp .inp:focus, .hcp .sel:focus, .hcp .ta:focus { border-color: #FFD700; box-shadow: 0 0 0 4px rgba(255,215,0,.1); background: #fff; }
.hcp .ta { resize: vertical; min-height: 95px; }
.hcp .ck-row { display: flex; align-items: center; gap: 8px; padding: 9px 0; }
.hcp .ck { width: 16px; height: 16px; accent-color: #0d0d0d; cursor: pointer; }
.hcp .ck-lbl { font-size: .85rem; font-weight: 600; color: var(--text); cursor: pointer; }
.hcp .submit-btn { width: 100%; padding: 14px; font-size: .93rem; font-weight: 800; background: linear-gradient(135deg, #FFD700, #e6c200); color: #0d0d0d; border: none; border-radius: 13px; cursor: pointer; font-family: 'Outfit', sans-serif; transition: all .2s; margin-top: 10px; box-shadow: 0 6px 22px rgba(255,215,0,.3); display: flex; align-items: center; justify-content: center; gap: 8px; }
...
.hcp .mpesa-amt-val { font-size: 1.6rem; font-weight: 900; color: #FFD700; }
...
.hcp .rev-craft-av { width: 30px; height: 30px; border-radius: 50%; border: 2px solid #FFD700; object-fit: cover; }
...
.hcp .pay-stat-icon i { color: #FFD700; font-size: .88rem; }
...
.hcp .prof-av { width: 68px; height: 68px; border-radius: 50%; border: 3px solid #FFD700; flex-shrink: 0; object-fit: cover; }
.hcp .prof-name { font-size: clamp(1.25rem, 3vw, 1.8rem); font-weight: 900; color: #fff; margin-bottom: 5px; letter-spacing: -.03em; }
.hcp .prof-role { display: inline-block; background: rgba(255,215,0,.12); color: #FFD700; border: 1px solid rgba(255,215,0,.25); border-radius: 50px; padding: 2px 11px; font-size: .62rem; font-weight: 700; text-transform: uppercase; letter-spacing: .08em; }
...
.hcp .prof-stat-v { font-size: 1.4rem; font-weight: 800; color: #FFD700; margin-bottom: 2px; }
...
.hcp .prof-item:hover { border-color: #FFD700; transform: translateY(-2px); }
...
.hcp .prof-item-icon i { color: #FFD700; font-size: .8rem; }
...
.hcp .modal-hd.light { background: linear-gradient(135deg, #fef9c3, #f0fdf4); }
...
.hcp .mbl-gold  { background: linear-gradient(135deg, #FFD700, #e6c200); color: #0d0d0d; box-shadow: 0 4px 14px rgba(255,215,0,.3); font-weight: 800; }
...
function SidebarContent({ client, tab, setTab, onClose }) {
  const TABS = [
...
export default function HireCraftsmanPage() {
  ...
  const initiateMpesa = async () => {
    ...
    if (!clean.match(/^(2547|2541|07|01)\d{7,8}$/)) {
      alert('Enter a valid M-Pesa number, e.g. 0712345678 or 254712345678'); return;
    }
    clearInterval(pollRef.current);
    setPayBusy(true); setPayState('idle'); setPayMsg('');
    try {
      await api.post(`/job-requests/${payJob.id}/pay/`, { phone: clean, amount: Number(payJob.budget) });
      setPayBusy(false); setPayState('stk_sent');
      ...
      pollRef.current = setInterval(async () => {
        ...
        try {
          ...
          if (s === 'COMPLETE') {
            clearInterval(pollRef.current); setPayState('complete');
            ...
            clearInterval(pollRef.current); setPayState('failed');
      ...
      setPayBusy(false); setPayState('failed');
  ...
  if (!client) return (
    <div style={{ minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center', background:'#0d0d0d', fontFamily:'Outfit,sans-serif' }}>
      <div style={{ textAlign:'center' }}>
        <div style={{ width:44, height:44, border:'3px solid rgba(255,215,0,.2)', borderTopColor:'#FFD700', borderRadius:'50%', animation:'hcp-spin .7s linear infinite', margin:'0 auto 14px' }}/>
        <p style={{ color:'rgba(255,215,0,.6)', fontWeight:700, fontSize:'.88rem' }}>Loading your dashboard…</p>
      </div>
    </div>
  );
